...
function calculateShortCircuitCurrent(voltage, impedance) {
  ...
  if (!impedance || typeof impedance !== 'object') {
    throw new Error('Impedance must be a valid object with real and imag properties');
  }
  if (typeof impedance.real !== 'number' || typeof impedance.imag !== 'number') {
...
function convertPowerUnits(power, fromUnit, toUnit) {
  ...
  if (!units.hasOwnProperty(fromUnit)) {
    throw new Error(`Invalid source unit: ${fromUnit}. Valid units: ${Object.keys(units).join(', ')}`);
  }
  if (!units.hasOwnProperty(toUnit)) {
    throw new Error(`Invalid target unit: ${toUnit}. Valid units: ${Object.keys(units).join(', ')}`);
  }
...
function validateElectricalParams(params) {
  ...
  if (params.impedance !== undefined) {
    ...
    if (typeof params.impedance.imag !== 'number' || isNaN(params.impedance.imag)) {
      errors.push('Reactance must be a valid number (can be negative for capacitive)');
    }
  }
