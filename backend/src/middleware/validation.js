...
function validateICCInput(req, res, next) {
  try {
    ...
    if (
      voltage !== undefined &&
      (typeof voltage !== 'number' || !isFinite(voltage))
    ) {
...
function validateFeederInput(req, res, next) {
  try {
    ...
    // Validate current
    if (
      typeof body.I_base !== 'number' ||
      !isFinite(body.I_base) ||
      body.I_base <= 0
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (body.ambientC !== undefined) {
      if (
        typeof body.ambientC !== 'number' ||
        !isFinite(body.ambientC) ||
        body.ambientC < -40 ||
        body.ambientC > 100
      ) {
        return res.status(400).json({
    ...
    if (body.Icu_kA !== undefined) {
      if (
        typeof body.Icu_kA !== 'number' ||
        !isFinite(body.Icu_kA) ||
        body.Icu_kA <= 0
      ) {
        return res.status(400).json({
          success: false,
    ...
    if (body.Isc_kA !== undefined) {
      if (
        typeof body.Isc_kA !== 'number' ||
        !isFinite(body.Isc_kA) ||
        body.Isc_kA <= 0
      ) {
        return res.status(400).json({
          success: false,
    ...
    for (let i = 0; i < body.edges.length; i++) {
      ...
      if (edge.data && typeof edge.data === 'object') {
        ...
        if (edge.data.longitud !== undefined) {
          if (
            typeof edge.data.longitud !== 'number' ||
            !isFinite(edge.data.longitud) ||
            edge.data.longitud <= 0
          ) {
            return res.status(400).json({
              success: false,
        ...
        if (
          edge.data.material !== undefined &&
          !['Cu', 'Al'].includes(edge.data.material)
        ) {
          return res.status(400).json({
            success: false,
