const proteccionService = require('../../modules/protections/proteccion.service')
const {
  tccSchema,
  disparoSchema,
  coordinacionSchema,
  coordinacionCascadaSchema,
  constantesCurvaSchema,
  seleccionSQDSchema,
  curvaSQDSchema,
  disparoSQDSchema,
  coordinacionSQDSchema,
  coordinacionCascadaSQDSchema,
} = require('../../validators/proteccion.schema')
...
exports.seleccionarCoordinacionCascada = asyncHandler(async (req, res) => {
  ...
  const result = await proteccionService.seleccionarCoordinacionCascada(
    req.body
  )
  success(res, result)
})
