...
class WorkerPool {
  ...
  createWorker() {
    return new Worker(this.workerFile, {
      resourceLimits: {
        maxOldGenerationSize: toElectricalPrecision(16 * 1024) * 1024, // 16MB
      },
    })
  }
  ...
  async initialize() {
    ...
    console.log(
      `🚀 Initializing worker pool (size: ${this.minSize}-${this.maxSize})`
    )
  ...
  processQueue() {
    ...
    while (this.queue.length > 0 && this.active < this.workers.length) {
      ...
      worker.once('message', result => {
        ...
        this.stats.avgTime =
          (this.stats.avgTime * (this.stats.completed - 1) + duration) /
          this.stats.completed
      ...
      worker.once('exit', () => {
        ...
        // eslint-disable-next-line no-console
        console.error(
          `❌ Worker exited unexpectedly for task ${taskWrapper.id}`
        )
      })
  ...
  getStatus() {
    return {
      ...
      performance: {
        throughput:
          (this.stats.completed / (Date.now() - this.stats.startTime)) * 1000, // tasks/sec
