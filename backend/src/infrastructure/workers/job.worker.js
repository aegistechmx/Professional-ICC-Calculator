  ...
  switch (type) {
    ...
    case 'contingency':
      ...
      for (const contingency of contingencies) {
        ...
        const pfResult = solveFDLF(modified, {
          tolerance: 1e-6,
          maxIterations: 20,
        })
      ...
      result = {
        ...
        summary: {
          ...
          critical: results.filter(r => r.security.severity === 'critical')
            .length,
        },
      }
      break
...
async function runMonteCarlo(system, scenarios) {
  ...
  console.log(
    `🎲 Running Monte Carlo analysis with ${scenarios.length} scenarios...`
  )
  ...
  for (const scenario of scenarios) {
    ...
    // Run power flow
    const pfResult = solveFDLF(modifiedSystem, {
      tolerance: 1e-6,
      maxIterations: 20,
    })
  ...
  // eslint-disable-next-line no-console
  console.log(
    `🎲 Monte Carlo analysis complete: ${overallStats.successful}/${results.length} scenarios`
  )
...
function calculateMonteCarloStats(results) {
  ...
  const loadGenImbalances = results.map(r =>
    r.metrics ? r.metrics.loadGeneration : 0
  )
  const meanImbalance =
