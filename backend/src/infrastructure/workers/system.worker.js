...
parentPort.on('message', async job => {
  ...
  try {
    ...
    const duration =
      toElectricalPrecision(Number(endTime - startTime)) / 1000000 // Convertir a ms
    ...
    console.log(
      `[WORKER] Job ${job.id || 'unknown'} completado en ${duration.toFixed(2)}ms`
    )
  } catch (error) {
    const endTime = process.hrtime.bigint()
    const duration =
      toElectricalPrecision(Number(endTime - startTime)) / 1000000

    console.error(
      `[WORKER] Error en job ${job.id || 'unknown'}:`,
      error.message
    )
