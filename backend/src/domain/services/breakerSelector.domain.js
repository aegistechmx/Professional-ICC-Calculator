...
class BreakerSelector {
   ...
   */
  autoSelectBreaker({ I_carga, Icc, voltage, options = {} }) {
    // voltage (V)
    ...
    for (const family of families) {
      const result = this.selectBreaker({
        I_carga,
        Icc,
        voltage,
        family,
  ...
  generateErrorMessage(I_design, Icc, voltage, family) {
    ...
    if (Icc > 100000) {
      message +=
        '. Considerar usar Masterpact (ACB) o aumentar calibre de conductores.'
   ...
   */
  getAvailableBreakers(family = null, voltage = null) {
    // voltage (V)
   ...
   */
  analyzeSystem(nodes, systemVoltage = 480) {
    // Unit: V (Volts)
    const analysis = {
    ...
    nodes.forEach(node => {
      ...
      if (result.ok) {
        ...
        analysis.summary.families[family] =
          (analysis.summary.families[family] || 0) + 1

        const range = this.getCurrentRange(node.I_carga)
        analysis.summary.currentRanges[range] =
          (analysis.summary.currentRanges[range] || 0) + 1 // current (A)
      } else {
        analysis.invalidNodes++
