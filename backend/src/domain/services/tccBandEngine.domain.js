...
class TCCBandEngine {
  ...
  autoCoordinateBands(downstream, upstream, margin = 0.3, maxAttempts = 15) {
    ...
    while (attempts < maxAttempts) {
      ...
      // Ajustar upstream
      const adjustment = this.adjustUpstream(
        upstream,
        downstream,
        check.worstConflict
      )
    ...
    return {
      ...
      attempts,
      history,
      remainingConflicts: this.checkBandCoordination(
        downstream,
        upstream,
        margin
      ).conflicts,
  ...
  adjustUpstream(upstream, downstream, _conflict) {
    ...
    if (upstream.delay !== undefined && upstream.delay < 2.0) {
      upstream.delay = toElectricalPrecision(
        parseFloat(upstream.delay + 0.1).toFixed(2)
      )
    ...
    // Prioridad 2: Ajustar curva base (TMS)
      ...
      upstream.TMS = toElectricalPrecision(
        parseFloat(upstream.TMS * 1.15).toFixed(3)
      )
    ...
    // Prioridad 3: Aumentar pickup
      ...
      upstream.pickup !== undefined &&
      upstream.pickup < downstream.pickup * 2
    ) {
      upstream.pickup = toElectricalPrecision(
        parseFloat(upstream.pickup * 1.2).toFixed(1)
      )
    ...
    // Prioridad 4: Ajustar instantáneo
      ...
      upstream.instantaneous.min = toElectricalPrecision(
        parseFloat(upstream.instantaneous.min * 1.1).toFixed(0)
      )
      upstream.instantaneous.max = toElectricalPrecision(
        parseFloat(upstream.instantaneous.max * 1.1).toFixed(0)
      )
  ...
  generateTCCBand(
    ...
    for (
      let I = breaker.rating * startMultiplier;
      I <= breaker.rating * endMultiplier;
      I *= step
    ) {
      ...
      if (band) {
        dataMin.push({
          x: toElectricalPrecision(parseFloat(I.toFixed(1))),
          y: band.min,
          zone: band.zone,
        })
        dataMax.push({
  ...
  calculateOverlap(downstream, upstream) {
    ...
    for (const I of currents) {
      ...
      const overlap = Math.max(
        0,
        Math.min(d.max, u.max) - Math.max(d.min, u.min)
      )
