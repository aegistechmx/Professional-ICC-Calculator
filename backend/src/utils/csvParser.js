...
function parseCSV(filePath) {
  try {
    ...
    const startIndex = lines[0].toLowerCase().includes('corriente') || 
                       lines[0].toLowerCase().includes('current') ? 1 : 0;
    ...
    for (let i = startIndex; i < lines.length; i++) {
      ...
      if (parts.length >= 2) {
        const corriente = parseFloat(parts[0]);
        const tiempo = parseFloat(parts[1]);
        
        if (!isNaN(corriente) && !isNaN(tiempo) && corriente > 0 && tiempo > 0) {
          puntos.push({
            corriente,
            tiempo
          });
    ...
    console.error(`Error parseando CSV ${filePath}:`, error.message);
    return [];
  }
}
...
function loadCurvesFromDirectory(dirPath) {
  ...
  try {
    ...
    console.error(`Error cargando curvas de ${dirPath}:`, error.message);
  }
