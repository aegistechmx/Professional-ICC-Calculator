...
const { calculateOperatingTime, generateTCCCurve } = require('./TCCCurves')
...
function analyzeCoordination(upstream, downstream, options = {}) {
  const {
    faultCurrents = [
      downstream.pickup * 2,
      downstream.pickup * 10,
      downstream.pickup * 20,
    ],
  ...
  faultCurrents.forEach(current => { // current (A)
    ...
    // current (A)
    const t_downstream = downstream.getOperatingTime(current) // current (A)
    // current (A)
...
function autoAdjustSettings(devices, options = {}) {
  const {
    faultCurrent = 10000, // Unit: A (Amperes)
    coordinationMargin = 0.2,
    maxTMS = 1.0,
    minTMS = 0.05,
  } = options
...
function generateCoordinationReport(analysis) {
  const { overallCoordinated, coordinationResults, selectivityMatrix } =
    analysis
