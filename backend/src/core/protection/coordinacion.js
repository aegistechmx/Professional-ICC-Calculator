...
function evaluarCoordinacion({
  ...
  for (let i = 0; i < curva1.length; i++) {
    ...
    const p2 = curva2.find(
      p => Math.abs(p.corriente - p1.corriente) < toleranciaCorriente // current (A)
    )
 ...
 */
function evaluarCoordinacionCascada({ curvas, margen = 0.2 }) {
  if (!Array.isArray(curvas) || curvas.length < 2) {
    throw new Error(
      'Se requieren al menos 2 curvas para coordinación en cascada'
    )
  }
 ...
 */
function encontrarCruce({ curva1, curva2 }) {
  for (let i = 0; i < curva1.length; i++) {
    const p1 = curva1[i]
...
function generarReporteCoordinacion(resultadoCoordinacion) {
  const { coordinado, problemas, margenAplicado, totalPuntosAnalizados } =
