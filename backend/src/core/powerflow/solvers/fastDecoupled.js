...
class FastDecoupledSolver {
  ...
  buildDecoupledMatrices(system) {
    ...
    const bPrime = Array(n)
      .fill()
      .map(() => Array(n).fill(0))
    const bDoublePrime = Array(n)
      .fill()
      .map(() => Array(n).fill(0))
    ...
    system.branches.forEach(branch => {
      ...
      const b =
        -1 /
        Math.sqrt(
          branch.impedance.real * branch.impedance.real +
            branch.impedance.imag * branch.impedance.imag
        )
  ...
  calculateMismatches(voltages, system) {
    ...
    system.buses.forEach((bus, i) => {
      ...
      if (bus.type === 'pq') {
        mismatches[index++] =
          bus.power - this.calculateRealPower(V, i, voltages, system)
        mismatches[index++] =
          bus.reactive - this.calculateReactivePower(V, i, voltages, system)
      } else if (bus.type === 'pv') {
        mismatches[index++] =
          bus.power - this.calculateRealPower(V, i, voltages, system)
      }
