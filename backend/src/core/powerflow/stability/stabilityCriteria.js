...
function checkSpeedStability(generators, options = {}) {
  ...
  generators.forEach(gen => {
    ...
    if (
      speedDeviation > maxSpeedDeviation ||
      gen.omega < minSpeed ||
      gen.omega > maxSpeed
    ) {
      stable = false
...
function checkEqualAreaCriterion(trajectory, options = {}) {
  const {
    ...
    _timeWindow = 1.0, // seconds to analyze
  } = options
  ...
  return {
    ...
    angleLimit,
    reason: stable
      ? 'Equal area criterion satisfied'
      : 'Equal area criterion violated',
  }
}
...
function checkKineticEnergy(generators, options = {}) {
  ...
  generators.forEach(gen => {
    ...
    const kineticEnergy =
      0.5 * (gen.inertia || 5.0) * speedDeviation * speedDeviation
 ...
 */
function checkSystemStability(generators, _trajectory = [], options = {}) {
  const {
  ...
  if (criteria.includes('angle')) {
    ...
    results.criticalGenerators.push(
      ...results.criteria.angle.criticalGenerators
    )
  }
  ...
  if (criteria.includes('speed')) {
    ...
    results.criticalGenerators.push(
      ...results.criteria.speed.criticalGenerators
    )
  }
  ...
  if (criteria.includes('energy')) {
    ...
    results.criticalGenerators.push(
      ...results.criteria.energy.criticalGenerators
    )
  }
