    ...
    for (let jIdx = 0; jIdx < angleIndex.length; jIdx++) {
      ...
      if (i === j) {
        ...
        for (let k = 0; k < n; k++) {
          ...
          sum +=
            Vi * Vk * (-G[i][k] * Math.sin(theta) + B[i][k] * Math.cos(theta))
        }
        J[row][col] = sum
      } else {
        // Off-diagonal
    ...
    for (let jIdx = 0; jIdx < voltageIndex.length; jIdx++) {
      ...
      if (i === j) {
        ...
        for (let k = 0; k < n; k++) {
          ...
          sum += Vk * (G[i][k] * Math.cos(theta) + B[i][k] * Math.sin(theta))
        }
    ...
    for (let jIdx = 0; jIdx < angleIndex.length; jIdx++) {
      ...
      if (i === j) {
        ...
        for (let k = 0; k < n; k++) {
          ...
          sum +=
            Vi * Vk * (G[i][k] * Math.cos(theta) + B[i][k] * Math.sin(theta))
        }
        J[row][col] = -sum
      } else {
        // Off-diagonal
    ...
    for (let jIdx = 0; jIdx < voltageIndex.length; jIdx++) {
      ...
      if (i === j) {
        ...
        for (let k = 0; k < n; k++) {
          ...
          sum += Vk * (G[i][k] * Math.sin(theta) - B[i][k] * Math.cos(theta))
        }
        J[row][angleIndex.length + jIdx] = sum - 2 * Vi * B[i][i]
      } else {
        // Off-diagonal
        J[row][angleIndex.length + jIdx] =
          Vi * (G[i][j] * Math.sin(theta) - B[i][j] * Math.cos(theta))
      }
    }
