...
function generateN1Contingencies(system) {
  ...
  system.buses.forEach((bus, i) => {
    if (bus.type === 'PV' && bus.P > 0) { // Only active generators
      contingencies.push({
...
function generateRankedContingencies(system, options = {}) {
  ...
  if (includeLines) {
    system.branches.forEach((branch, i) => {
      ...
      const severity = estimatedLoading > 0.8 ? 'high' : 
                      estimatedLoading > 0.6 ? 'medium' : 'low';
  ...
  if (includeGenerator) {
    system.generators.forEach((gen, i) => {
      if (gen.P > 0) {
        ...
        const severity = gen.P > 0.5 ? 'high' : 
                        gen.P > 0.3 ? 'medium' : 'low';
