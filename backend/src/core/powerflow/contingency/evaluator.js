  ...
  result.voltages.forEach((v, i) => {
    ...
    if (magnitude < Vmin * 0.9 || magnitude > Vmax * 1.1) {
      violations.voltage.push({
        bus: i,
        magnitude,
        type: magnitude < Vmin * 0.9 ? 'critical_undervoltage' : 'critical_overvoltage'
      });
...
function rankContingencies(results) {
  return results.map(r => {
    const violationCount = (r.violations?.voltage?.length || 0) + 
...
function getSecurityIndex(results) {
  ...
  const secure = results.filter(r => 
    r.status === 'ok' && 
    !r.violations?.critical && 
    !r.violations?.marginal
  ).length;
...
function getCriticalContingencies(results) {
  return results.filter(r => 
    r.status !== 'ok' || r.violations?.critical
  );
}
