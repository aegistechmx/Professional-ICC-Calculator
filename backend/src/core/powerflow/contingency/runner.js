...
const { generateN1Contingencies, generateN2Contingencies } = require('./generator');
const { evaluateViolations, rankContingencies, getSecurityIndex, getCriticalContingencies } = require('./evaluator');
...
function runN1Contingency(model, options = {}) {
  ...
  for (const contingency of contingencies) {
    ...
    const violations = result.converged 
      ? evaluateViolations(result, contingencyModel, { Vmin, Vmax, loadingLimit })
  ...
  return {
    ...
    summary: {
      total: contingencies.length,
      critical: rankedResults.filter(r => r.status !== 'ok' || r.violations.critical).length,
      marginal: rankedResults.filter(r => r.violations.marginal && !r.violations.critical).length,
      secure: rankedResults.filter(r => r.status === 'ok' && !r.violations.critical && !r.violations.marginal).length,
