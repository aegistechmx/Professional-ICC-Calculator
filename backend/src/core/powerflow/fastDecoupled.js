...
function buildBprime(B, slackIndex) {
  ...
  const Bp = Array(size).fill(null).map(() => Array(size).fill(0));
...
function buildBdoubleprime(B, pqBuses) {
  ...
  const Bpp = Array(size).fill(null).map(() => Array(size).fill(0));
  ...
  for (let iter = 0; iter < maxIterations; iter++) {
    ...
    for (let i = 0; i < system.buses.length; i++) {
      ...
      for (let j = 0; j < system.buses.length; j++) {
        ...
        P[i] += Vi * Vj * (G[i][j] * Math.cos(theta) + B[i][j] * Math.sin(theta));
        Q[i] += Vi * Vj * (G[i][j] * Math.sin(theta) - B[i][j] * Math.cos(theta));
      }
    }
