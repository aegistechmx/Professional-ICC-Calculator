...
class StressTest {
  ...
  generateLargeSystem(systemConfig = {}) {
    ...
    const { ElectricalSystem, Bus, Line, Generator, Load } = require('../electrical/ElectricalSystem');
    
    const system = new ElectricalSystem();
    
    // Generate buses
    for (let i = 0; i < numBuses; i++) {
      const type = i === 0 ? 'Slack' : 
        i < numGenerators + 1 ? 'PV' : 'PQ';
      ...
      const bus = new Bus({
        ...
        generation: type === 'Slack' ? { P: 100, Q: 50 } :
          type === 'PV' ? { P: 50, Q: 0 } : { P: 0, Q: 0 },
        load: type === 'PQ' ? { P: Math.random() * 20, Q: Math.random() * 10 } : { P: 0, Q: 0 }
      });
  ...
  runPerformanceBenchmark(system, engines) {
    ...
    if (engines.fault) {
      ...
      const faultResult = engines.fault.calculate({ type: '3P', busId: system.buses[0].id });
  ...
  runStabilityValidation(system, engines) {
    ...
    if (engines.fault) {
      const faultResult = engines.fault.calculate({ type: '3P', busId: system.buses[0].id });
      stabilityMetrics.faultStable = faultResult !== null && !isNaN(faultResult.current);
    }
  ...
  runCompleteStressTest(engines) {
    ...
    const summary = {
      ...
      faults: {
        ...
        averageExecutionTime: faultResults.reduce((sum, f) => sum + (f.executionTime || 0), 0) / faultResults.length
      },
  ...
  generateRecommendations(results) {
    ...
    // Check performance
    if (results.performance.powerFlow && results.performance.powerFlow.executionTime > 1000) {
      recommendations.push({
        type: 'performance',
        severity: 'warning',
        message: 'Power flow execution time exceeds 1 second for large systems',
        suggestion: 'Consider implementing sparse matrix solvers for better performance'
      });
    }
