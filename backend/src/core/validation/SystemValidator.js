...
class SystemValidator {
  ...
  static validateSlackBus(system) {
    ...
    if (slackBuses.length === 0) {
      return {
        valid: false,
        error: 'No slack bus found in system. At least one slack bus is required for power flow analysis.'
      };
    }
  ...
  static validateBases(system) {
    ...
    const busesWithoutBase = system.buses.filter(b => !b.baseKV || b.baseKV <= 0);
  ...
  static buildAdjacencyList(system) {
    ...
    system.trafos.forEach(tranfo => {
      if (adjacency[tranfo.fromBus]) adjacency[tranfo.fromBus].push(tranfo.toBus);
