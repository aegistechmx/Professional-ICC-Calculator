...
class ConsistencyValidator {
  ...
  validateCurrentConsistency(results) {
    ...
    if (maxDiff < this.options.tolerance) {
      ...
      this.results.warnings.push({
        test: 'current_consistency',
        message: 'Currents differ between power flow and fault analysis (expected for fault conditions)',
        maxDifference: maxDiff
      });
    }
  }
  ...
  validateVoltageConsistency(results) {
    ...
    const voltagesDyn = results.dynamics.voltages ? results.dynamics.voltages[0] : null;
    ...
    if (maxDiff < this.options.tolerance) {
      ...
      this.results.failed.push({
        test: 'voltage_consistency',
        message: 'Initial voltages inconsistent between power flow and dynamics',
        maxDifference: maxDiff,
        tolerance: this.options.tolerance
      });
    }
   ...
   * @param {number} tolerance - Tolerance
   */
  assertCurrentsMatch(currentsA, currentsB, tolerance = this.options.tolerance) {
