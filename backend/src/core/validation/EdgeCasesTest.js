...
class EdgeCasesTest {
  ...
  testIslanding(system, engines) {
    try {
      ...
      const pfResult = engines.powerFlow ? engines.powerFlow.run(islandedSystem) : null;
      
      // Check voltage stability
      const voltageStable = pfResult ? pfResult.voltages.every(v => v > 0.85 && v < 1.15) : false;
  ...
  testLossOfGeneration(system, engines) {
    try {
      // Identify generators
      const generators = system.buses.filter(b => b.type === 'PV' || b.type === 'Slack');
      ...
      const voltageStable = pfResult ? pfResult.voltages.every(v => v > 0.9 && v < 1.1) : false;
      
      // Check if remaining generation can meet load
      const totalGeneration = system.buses.reduce((sum, b) => sum + (b.generation.P || 0), 0);
      const totalLoad = system.buses.reduce((sum, b) => sum + (b.load.P || 0), 0);
  ...
  testReconnection(system, engines) {
    try {
      ...
      // Remove line
      const removedLines = system.lines.filter(l => l.id !== lineToDisconnect.id);
      ...
      const voltageStable1 = pfResult1 ? pfResult1.voltages.every(v => v > 0.85 && v < 1.15) : false;
      const voltageStable2 = pfResult2 ? pfResult2.voltages.every(v => v > 0.95 && v < 1.05) : false;
      
      // Check if system recovered
      const recovered = voltageStable2 && (pfResult2 ? pfResult2.converged : false);
  ...
  runAllEdgeCases(system, engines) {
    ...
    const failedTests = Object.values(results).filter(r => r.success === false || r.overallStable === false);
  ...
  generateRecommendations(results) {
    ...
    if (results.islanding && !results.islanding.overallStable) {
      recommendations.push({
        ...
        suggestion: 'Implement automatic load shedding and frequency control for islanded operation'
      });
    }
    
    // Check loss of generation
    if (results.lossOfGeneration && !results.lossOfGeneration.generationSufficient) {
      recommendations.push({
        type: 'generation',
        severity: 'warning',
        message: 'Insufficient generation after loss of generator',
        suggestion: 'Implement spinning reserve requirements and automatic generation control'
      });
    }
    ...
    if (results.reconnection && !results.reconnection.recovered) {
      recommendations.push({
        ...
        suggestion: 'Implement synchronization checks and soft reconnection procedures'
      });
    }
