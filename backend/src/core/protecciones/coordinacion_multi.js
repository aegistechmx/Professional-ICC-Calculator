 ...
 * Margen típico: 0.2 - 0.4 segundos
 */
 ...
 */
function evaluarCoordinacionMulti({
  protecciones,
  margen = 0.2
}) {
  if (!Array.isArray(protecciones) || protecciones.length < 2) {
    throw new Error('Se requieren al menos 2 protecciones para evaluar coordinación');
  }
    ...
    for (const punto of curvaDown) {
      ...
      if (margenReal < margen) {
        problemas.push({
          ...
          margen_real: parseFloat(margenReal.toFixed(4)),
