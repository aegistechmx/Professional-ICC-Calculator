  ...
  for (const brk of sqdBreakers) {
    ...
    candidatos.push({
      ...
      validaciones: {
        ...
        margenIn: ((brk.In - corriente_carga) / corriente_carga * 100).toFixed(1) + '%'
      }
    });
  }
 ...
 */
function coordinarProteccionesSQD({
  downstream,
  upstream,
  margen = 0.2
}) {
 ...
 */
function seleccionarCoordinacionCascada({
  niveles,
  margen = 0.2
}) {
  if (!Array.isArray(niveles) || niveles.length < 2) {
    throw new Error('Se requieren al menos 2 niveles para coordinación en cascada');
  }
  ...
  // Resumen
  const totalCoordinado = resultadosCoordinacion.filter(r => r.coordinado).length;
