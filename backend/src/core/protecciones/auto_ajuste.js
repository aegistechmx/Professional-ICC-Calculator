...
function calcularAjustesRecomendados(protecciones, margen = 0.2) {
  ...
  for (const key in problemasPorPar) {
    ...
    const ajusteSugerido = Math.ceil(
      (margen - grupo.peorMargen + 0.05) / 0.05
    ) * 0.05; // Redondear a múltiplo de 0.05

    recomendaciones.push({
      par: grupo.par,
      nivel_up: grupo.nivel_up,
      peorMargen: parseFloat(grupo.peorMargen.toFixed(4)),
      margenObjetivo: margen,
      ajusteSugerido: parseFloat(ajusteSugerido.toFixed(2)),
  ...
  return {
    ...
    recomendaciones: recomendaciones.sort((a, b) => a.peorMargen - b.peorMargen)
  };
}
