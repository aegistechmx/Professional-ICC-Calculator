...
class PhaseGroundProtection {
  ...
  calculateSymmetricalComponents(phaseCurrents) {
    ...
    const I1 = (Ia_complex.add(Ib_complex.multiply(this.alpha())).add(Ic_complex.multiply(this.alpha().multiply(this.alpha())))).divide(3);
    const I2 = (Ia_complex.add(Ib_complex.multiply(this.alpha().multiply(this.alpha()))).add(Ic_complex.multiply(this.alpha()))).divide(3);
  ...
  toComplex(phasor) {
    ...
    return {
      ...
      add: (other) => ({
        ...
        magnitude: () => Math.sqrt((magnitude * Math.cos(angle) + other.real) ** 2 + (magnitude * Math.sin(angle) + other.imag) ** 2),
        angle: () => Math.atan2(magnitude * Math.sin(angle) + other.imag, magnitude * Math.cos(angle) + other.real) * 180 / Math.PI,
        divide: (scalar) => ({
        ...
        multiply: (other) => {
          ...
          return {
            real: r1 * r2 - i1 * i2,
            imag: r1 * i2 + i1 * r2,
            magnitude: () => magnitude * Math.sqrt(other.real ** 2 + other.imag ** 2),
            angle: () => (phasor.angle || 0) + Math.atan2(other.imag, other.real) * 180 / Math.PI
          };
        }
      })
    };
  ...
  determineFaultType(separated) {
    const { phase, ground, sequence } = separated;
...
class GroundFaultProtection extends PhaseGroundProtection {
  ...
  evaluateSensitive(currents) {
    ...
    const I_unbalanced = Math.abs(separated.phase.Ia - separated.phase.Ib) +
