...
class LSIGBreaker {
  ...
  evaluate(measurement, currentTime = 0) {
    ...
    if (longTimeResult.trip) {
      const totalDelay = longTimeResult.operatingTime + this.temporalDelays.relayPickupDelay + 
                        this.temporalDelays.breakerMechanicalDelay + this.temporalDelays.arcExtinctionDelay;
      trips.push({
        element: 'longTime',
    ...
    if (shortTimeResult.trip) {
      const totalDelay = shortTimeResult.operatingTime + this.temporalDelays.relayPickupDelay + 
                        this.temporalDelays.breakerMechanicalDelay + this.temporalDelays.arcExtinctionDelay;
      trips.push({
        element: 'shortTime',
    ...
    if (instantaneousResult.trip) {
      const totalDelay = instantaneousResult.operatingTime + this.temporalDelays.relayPickupDelay + 
                        this.temporalDelays.breakerMechanicalDelay + this.temporalDelays.arcExtinctionDelay;
      trips.push({
    ...
    if (groundResult.trip) {
      const totalDelay = groundResult.operatingTime + this.temporalDelays.relayPickupDelay + 
                        this.temporalDelays.breakerMechanicalDelay + this.temporalDelays.arcExtinctionDelay;
      trips.push({
        element: 'ground',
    ...
    if (overallTrip && !this.state.overallTrip) {
      ...
      this.state.relayPickupAt = currentTime + this.temporalDelays.relayPickupDelay;
      this.state.breakerCommandAt = trips[0].tripAt - this.temporalDelays.arcExtinctionDelay;
...
class LSIGBreakerCoordination extends LSIGBreaker {
   ...
   */
  evaluateWithCoordination(measurement, upstreamState = null, downstreamState = null) {
    ...
    if (upstreamState && upstreamState.tripTime) {
      const coordinationTime = upstreamState.tripTime - this.coordination.coordinationMargin;
    ...
    if (downstreamState && downstreamState.tripTime) {
      const coordinationTime = downstreamState.tripTime + this.coordination.coordinationMargin;
  ...
  autoAdjustForCoordination(upstreamBreaker = null, downstreamBreaker = null) {
    ...
    if (upstreamBreaker) {
      ...
      if (this.longTime.timeDelay >= upstreamLT.timeDelay) {
        const newDelay = upstreamLT.timeDelay - this.coordination.coordinationMargin;
    ...
    if (downstreamBreaker) {
      ...
      if (this.longTime.timeDelay <= downstreamLT.timeDelay) {
        const newDelay = downstreamLT.timeDelay + this.coordination.coordinationMargin;
        recommendations.push({
          element: 'L',
          setting: 'timeDelay',
          currentValue: this.longTime.timeDelay,
