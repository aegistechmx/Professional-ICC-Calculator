...
function runFullAnalysis(systemModel) {
  const { buses, branches, breakers, _loads, settings } = systemModel
...
function calculateBusICC(bus, branches, settings) {
  ...
  // Impedancia base
  const _Z_base = Math.pow(baseVoltage, 2) / (basePower * 1000000)
...
function getBranchISC(branch, buses) {
  ...
  const isc = Math.max(fromBus?.Isc || 5, toBus?.Isc || 5)
 ...
 */
function sortBreakersByHierarchy(breakers, _branches) {
