...
const MotorElectrico = (function() {
    ...
    function ejecutarMotorElectrico(input) {
        const sistema = normalizarInput(input);
        
        // 1. AMPACIDAD BASE
        const ampacidad = calcularAmpacidad(sistema);
        
        // 2. SELECCIÓN DE CONDUCTOR
        const conductor = seleccionarConductor({
        ...
        // 4. CORTOCIRCUITO
        const falla = calcularCortocircuito(sistema);
        
        // 5. PROTECCIÓN
        const proteccion = seleccionarProteccion({
            sistema: sistema,
            falla: falla,
            conductor: conductor
        });
        ...
        // 6. COORDINACIÓN
        const coordinacion = coordinarProtecciones({
            proteccion: proteccion,
            sistema: sistema
        });
        
        // 7. VALIDACIÓN NOM
        const validacion = validarNOM({
            sistema: sistema,
            conductor: conductor,
            proteccion: proteccion,
            ampacidad: ampacidad
