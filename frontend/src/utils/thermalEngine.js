...
export function updateThermalLayer(edges, results) {
  return edges.map(edge => {
    ...
    if (edge.thermal) {
      ...
      thermal.T =
        edge.thermal.T * (1 - smoothingFactor) + thermal.T * smoothingFactor
      thermal.loading =
        edge.thermal.loading * (1 - smoothingFactor) +
        thermal.loading * smoothingFactor
    }
  ...
  edges.forEach(edge => {
    ...
    switch (status) {
      case 'safe':
        ...
        break
      case 'normal':
        analysis.normalEdges++
        break
      case 'caution':
        analysis.cautionEdges++
        break
      case 'warning':
        analysis.warningEdges++
        break
      case 'danger':
        analysis.dangerEdges++
        break
      case 'critical':
