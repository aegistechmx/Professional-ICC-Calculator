    ...
    relays.forEach(relay2 => {
      if (relay1.id === relay2.id) {
        ...
        if (isUpstream || isDownstream) {
          ...
          const t1 = flow1
            ? calculateTripTimeHelper(flow1.IkA, relay1)
            : Infinity
          const t2 = flow2
            ? calculateTripTimeHelper(flow2.IkA, relay2)
            : Infinity
...
export function generateLoadingSummary(buses, flows) {
  ...
  const totalGeneration = buses.reduce(
    (sum, b) => sum + Math.max(0, b.P_final_MW || 0),
    0
  )
...
export function generateDashboardData(simulationResult) {
  ...
  return {
    ...
    coordinationMatrix: relays
      ? generateCoordinationMatrix(relays, flows)
      : null,
