...
export const ProfessionalBreakerLibrary = {
  ...
  QO: {
    ...
    // Modelos disponibles
    models: [
      15, 20, 25, 30, 35, 40, 50, 60, 70, 80, 90, 100, 110, 125, 150, 200, 225,
    ],
  },
...
export function createBreaker(family, In, id = null) {
  ...
  if (breakerDef.models && !breakerDef.models.includes(In)) {
    if (import.meta.env.DEV) {
      ...
      console.warn(
        `In=${In}A no está disponible para ${family}, modelos disponibles: ${breakerDef.models.join(', ')}`
      )
    }
  }
...
function generateRecommendations(breakers, validation) {
  ...
  if (!validation.compatible) {
    recommendations.push({
      ...
      severity: 'error',
      message:
        'El sistema tiene violaciones de compatibilidad que deben corregirse',
    })
  }
  ...
  if (hasElectronic && hasThermal) {
    recommendations.push({
      ...
      severity: 'warning',
      message:
        'Mezcla de breakers térmicos y electrónicos puede afectar coordinación',
    })
  }
  ...
  if (smallBreakers > breakers.length * 0.7) {
    recommendations.push({
      ...
      severity: 'info',
      message:
        'Considerar consolidar breakers pequeños para mejorar selectividad',
    })
  }
