...
export function detectCross(curveUp, curveDown, margin = 0.1) {
  ...
  return {
    ...
    crosses,
    worstMargin:
      crosses.length > 0 ? Math.min(...crosses.map(c => c.margin)) : 0,
...
export function autoCoordinate(upstream, downstream, options = {}) {
  ...
  while (iterations < maxIterations && !coordinated) {
    ...
    const adjustment = applyIntelligentAdjustment(
      upstream,
      worstCross,
      adjustmentStep,
      preserveSensitivity
...
function applyIntelligentAdjustment(
  breaker,
  cross,
  step = 0.1,
  preserveSensitivity = true
  ...
  switch (upstreamZone) {
    case 'INST':
      ...
      if (
        newIi <= ratings.In * 25 &&
        (!preserveSensitivity || newIi <= ratings.In * 15)
      ) {
        ratings.Ii = newIi
        adjusted = true
        adjustment = {
    ...
    analysis.push({
      ...
      averageTripTime:
        sensitivities.reduce(
          (sum, s) => sum + (s.tripTime === Infinity ? 0 : s.tripTime),
          0
        ) / faultCurrents.length,
    })
  })
