...
export function calculateTotalMotorContribution(nodes) {
  ...
  const totalIsc = motorContributions.reduce((sum, m) => 
    sum + (m.shortCircuitCurrent_ka || 0), 0);
  ...
  if (validMotors.length > 0) {
    ...
    const sumAdmittance = validMotors.reduce((sum, m) => 
      sum + (1 / m.Z_motor_ohm), 0);
  ...
  return {
    ...
    // Summary statistics
    largestMotor: motorContributions.reduce((max, m) => 
      m.shortCircuitCurrent_ka > (max?.shortCircuitCurrent_ka || 0) ? m : max, null),
...
export function combineFaultCurrents(utilityIsc_ka, motorIsc_ka) {
  ...
  return {
    ...
    motorContributionPercent: utilityIsc_ka > 0 ? 
      ((motorIsc_ka / totalFirstCycle) * 100).toFixed(1) : 0
  };
}
 ...
 */
export function calculateFaultCurrentWithMotors(nodes, edges, utilityIsc_ka = null) {
  ...
  if (sourceIsc === null) {
    ...
    const sources = nodes.filter(n => 
      n.type === 'transformer' || n.type === 'generator'
    );
  ...
  const combined = combineFaultCurrents(sourceIsc, motorData.totalContribution_ka);
