...
export class TCCEngine {
  ...
  static autoCoordinate(upstream, downstream, options = {}) {
    const { maxIterations = 20, margin = 0.1, adjustmentStep = 0.1 } = options
  ...
  static coordinateSystem(breakers, options = {}) {
    ...
    return {
      ...
      totalAdjustments: results.reduce(
  ...
  // ========================================
  static analyzeCoordination(
    breakers,
    faultCurrents = [1000, 2000, 5000, 10000]
  ) {
    const analysis = {
      totalPairs: 0,
      coordinatedPairs: 0,
    ...
    analysis.coordinationScore =
      (analysis.coordinatedPairs / analysis.totalPairs) * 100
    ...
    if (analysis.coordinationScore < 80) {
      analysis.recommendations.push({
        ...
        severity: 'warning',
        message:
          'Se recomienda ejecutar auto-coordinación para mejorar selectividad',
      })
    }
