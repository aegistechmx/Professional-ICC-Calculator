...
export function extractBranches(nodes, edges) {
  ...
  sourceNodes.forEach(source => {
    ...
    function dfs(nodeId, currentPath) {
      ...
      if (node.type === 'load' || node.type === 'motor' || node.type === 'panel') {
        const branchEdges = [];
        for (let i = 0; i < currentPath.length - 1; i++) {
          const edge = edges.find(e => 
            e.source === currentPath[i].id && e.target === currentPath[i + 1].id
          );
        ...
        branches.push({
          ...
          nodes: [...currentPath],
          edges: branchEdges,
          totalLength: branchEdges.reduce((sum, e) => sum + (e.data?.longitud || 0), 0),
 ...
 */
export function calculateBranchImpedance(branch, nodes) {
  let totalR = 0;
  let totalX = 0;
...
function getConductorImpedance(calibre, material) {
  ...
  const impedances = {
    ...
    'aluminum': {
      ...
      '2/0': { R: 0.2230, X: 0.0832 },
    }
  };

  const mat = material.toLowerCase() === 'aluminio' || material.toLowerCase() === 'al' 
    ? 'aluminum' 
    : 'copper';
