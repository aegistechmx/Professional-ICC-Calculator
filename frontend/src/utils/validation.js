...
export function validateHierarchy(nodes, edges) {
  ...
  transformers.forEach(t => {
    if (outDegree[t.id] === 0) {
      errors.push(
        `Transformador '${t.data.label}' no tiene conexiones de salida.`
      )
    }
  })
  ...
  atsNodes.forEach(ats => {
    if (inDegree[ats.id] === 0) {
      errors.push(
        `ATS '${ats.data.label}' no tiene fuente de alimentación conectada.`
      )
    }
    if (outDegree[ats.id] === 0) {
      errors.push(
        `ATS '${ats.data.label}' no tiene salida conectada (debe conectar a breaker o panel).`
      )
    }
    // ATS should have at least 2 inputs (normal + emergency) for redundancy
    if (inDegree[ats.id] < 2) {
      errors.push(
        `Advertencia: ATS '${ats.data.label}' debería tener 2 fuentes (transformador + generador) para transferencia automática.`
      )
    }
  })
...
export function validateSystem(nodes, edges) {
  ...
  const allWarnings = [...cables.warnings, ...breakers.warnings]
