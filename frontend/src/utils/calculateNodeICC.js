...
export function calculateAccumulatedImpedance(nodes, edges) {
  ...
  if (sources.length === 0) {
    return { error: 'No source nodes found (transformer, generator, or utility)' };
  }

  // Initialize sources with their voltage
  sources.forEach(source => {
    const voltage = source.data?.parameters?.secundario || 
                   source.data?.parameters?.voltaje || 
                   source.data?.parameters?.tension || 480;
...
export function calculateNodeFaultCurrents(nodeImpedance) {
  ...
  Object.entries(nodeImpedance).forEach(([nodeId, data]) => {
    ...
    const I_sc_3f = Z_total > 0.001 ? 
      (data.sourceVoltage / (Math.sqrt(3) * Z_total)) : 0;
    
    // Calculate 1-phase fault current (approximate): I_sc_1f = V / (2 * Z)
    const I_sc_1f = Z_total > 0.001 ?
      (data.sourceVoltage / (2 * Z_total)) : 0;
...
export function calculateICCPerNode(nodes, edges) {
  ...
  const faultCurrents = calculateNodeFaultCurrents(impedanceResult.nodeImpedance);
  ...
  return {
    ...
    summary: {
      ...
      maxIsc: Math.max(...Object.values(faultCurrents).map(d => d.isc_3f_ka || 0)),
      minIsc: Math.min(...Object.values(faultCurrents)
