...
export function pasteSelection(clipboard, nodes, edges, options = {}) {
  ...
  const { viewport = null, mousePosition = null, offset = null } = options
...
export function clampToViewport(position, viewport) {
  ...
  return {
    x: Math.max(padding, Math.min(position.x, viewport.width - padding)),
    y: Math.max(padding, Math.min(position.y, viewport.height - padding)),
  }
}
