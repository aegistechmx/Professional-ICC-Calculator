 ...
 */
export function calculateManhattanRoute(
  sourceNode,
  targetNode,
  allNodes,
  existingEdges,
  ...
  const gridPath = aStar(grid, borderStart, borderEnd, cableMap, {
    avoidNodes,
    minimizeCrossings,
  })
...
export function optimizeAllRoutes(nodes, edges, canvasSize, options = {}) {
  ...
  const edgesWithFeeders = assignFeeders ? assignFeederIds(edges) : edges
