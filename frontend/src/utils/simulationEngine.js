    ...
    upstreamEdges.forEach(edge => {
      ...
      if (sourceNode && !visited.has(sourceNode.id)) {
        propagation.push({
          ...
          faultCurrent:
            results?.flujos?.find(
              f => f.from === sourceNode.id && f.to === currentNode
            )?.I || 0,
...
export function generateTCCTimeCurrentCurve(
  pickup,
  instantaneous,
  curveType = 'IEC'
) {
  ...
  Object.keys(nodesByLevel)
    ...
    .forEach((level, levelIndex) => {
      ...
      nodesInLevel.forEach((node, nodeIndex) => {
        const x =
          (nodeIndex - nodesInLevel.length / 2) * horizontalSpacing + 400
...
export function validateElectricalParameters(node) {
  ...
  if (
    node.data?.voltaje &&
    (node.data.voltaje < 100 || node.data.voltaje > 35000)
  ) {
  ...
  if (
    node.data?.I_carga &&
    (node.data.I_carga < 0 || node.data.I_carga > 10000)
  ) {
  ...
  if (
    node.data?.longitud &&
    (node.data.longitud < 0 || node.data.longitud > 1000)
  ) {
...
export function validateGraphConnectivity(nodes, edges) {
  ...
  edges.forEach(edge => {
    if (!nodeIds.has(edge.source)) {
      connectedNodes.add(
        `Edge ${edge.id}: nodo fuente ${edge.source} no existe`
      )
    }
    if (!nodeIds.has(edge.target)) {
      connectedNodes.add(
        `Edge ${edge.id}: nodo destino ${edge.target} no existe`
      )
    }
  })
