    ...
    while (queue.length > 0) {
      ...
      connectedEdges.forEach(edge => {
        ...
        if (!visited.has(nextNode) && !this.visitedEdges.has(edgeId)) {
          ...
          const propagationProbability = Math.min(
            1,
            this.speed * this.time * (1 - currentDistance * 0.1)
          )

          if (Math.random() < propagationProbability) {
            this.visitedEdges.add(edgeId)

            const faultLevel = Math.min(
              1,
              this.time * 0.5 * (1 - currentDistance * 0.05)
            )
...
export function createFaultSimulationLoop(canvas, graph, results, simulation) {
  ...
  const loop = () => {
    ...
    // const updatedEdges = graph.edges.map(edge => { // Unused variable removed
    graph.edges.forEach(edge => {
      let updatedEdge = { ...edge }

      // Aplicar efectos de falla
      const faultEdge = affectedEdges.find(
        f =>
