...
export function adjustPair(down, up, Icc, margin = 0.3, limits = {}) {
  const { TMS_max = 2.0, pickup_max_factor = 2.0, maxIterations = 50 } = limits
    ...
    if (!adjusted) {
      ...
      if (import.meta.env.DEV) {
        if (import.meta.env.DEV) {
          ...
          console.log(
            `No se pudo coordinar ${up.id} con ${down.id} para Icc=${Icc}A`
          )
        }
      }
    }
...
export function autoTuneProtection(graph, faultNode, IccMap, opts = {}) {
  ...
  for (let iter = 0; iter < maxIterations; iter++) {
      ...
      for (let i = 0; i < edges.length - 1; i++) {
        ...
        if (result.adjusted) {
          iterationAdjustments.push({
            ...
            afterDown: {
              TMS: edgeDown.breaker.TMS,
              pickup: edgeDown.breaker.pickup,
            },
    ...
    if (import.meta.env.DEV) {
      // eslint-disable-next-line no-console
      console.log(
        `Iteración ${iter + 1}: Error = ${iterationError.toFixed(4)}, Ajustes = ${iterationAdjustments.length}`
      )
    }
  }
...
export function generateTuningReport(result, validationResults) {
  ...
  if (!result.converged) {
    report.recommendations.push({
      ...
      severity: 'warning',
      message:
        'El auto-tuning no convergió. Considerar aumentar márgenes o revisar topología.',
    })
  }

  // Analizar ajustes de pickup
  const pickupAdjustments = result.adjustments.filter(
    adj =>
  ...
  const tmsAdjustments = result.adjustments.filter(
    adj => adj.afterUp.TMS !== adj.beforeUp.TMS
  )
...
export function integrateWithCoordinationEngine(
  coordinationEngine,
  autoTuningResult
) {
