...
export function useParticleEngine({
  ...
  const getEdgeCurrent = useCallback(
    edge => {
      if (!results) return 0

      // Buscar en diferentes estructuras de resultados
      const edgeResult =
        results?.data?.graphAnalysis?.edges?.find(
          e =>
  ...
  const getGlowIntensity = useCallback((edge, results) => {
    const edgeData =
      ...
      results?.flujos?.find(e => e.from === edge.source && e.to === edge.target)
  ...
  const getGlowStyle = useCallback(
    (edge, results, time = 0) => {
      ...
      if (intensity > 0.7)
        color = '#ff3b30' // Rojo - alto ICC
      else if (intensity > 0.4)
        color = '#ffcc00' // Amarillo - ICC medio
  ...
  const getStats = useCallback(() => {
    return {
      ...
      averageSpeed:
        particlesRef.current.reduce((sum, p) => sum + p.speed, 0) /
...
export function getEdgePathForGraph(edge, nodes) {
  ...
  if (edge.points && edge.points.length > 0) {
    return [sourceNode.position, ...edge.points, targetNode.position]
  }
