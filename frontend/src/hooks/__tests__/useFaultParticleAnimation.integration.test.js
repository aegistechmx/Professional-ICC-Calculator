...
describe('useFaultParticleAnimation Hook Integration Tests', () => {
  ...
  describe('Hook Initialization', () => {
    ...
    test('should initialize particle engine when canvas ref is set', async () => {
      ...
      expect(
        result.current.particleEngine === null ||
          typeof result.current.particleEngine === 'object'
      ).toBe(true)
    })
  ...
  describe('Fault Animation Lifecycle', () => {
    ...
    test('should handle breaker trip during animation', async () => {
      ...
      // Handle breaker trip
      act(() => {
        result.current.handleBreakerTrip(
          'breaker1',
          { tripTime: 0.1 },
          mockGraph
        )
...
describe('API Methods', () => {
  test('should update particle options correctly', async () => {
    ...
    // May not be called if engine is null
    if (result.current.particleEngine) {
      expect(result.current.particleEngine.updateOptions).toHaveBeenCalledWith(
        newOptions
      )
    }
  })
  ...
  test('should export frame correctly', async () => {
    ...
    if (result.current.particleEngine) {
      ...
      expect(
        frameData === undefined || frameData === 'data:image/png;base64,test'
      ).toBe(true)
    }
  })
...
describe('Backward Compatibility', () => {
  test('should provide compatibility methods', () => {
    ...
    expect(result.current.startFaultAnimation).toBe(
      result.current.startFaultParticleAnimation
    )
    expect(result.current.stopAnimation).toBe(
      result.current.stopParticleAnimation
    )
  })
})
...
describe('React Dependencies', () => {
  test('should handle graph changes correctly', async () => {
    const { result, rerender } = renderHook(
      ({ graph }) =>
        useFaultParticleAnimation(graph, mockOnNodeUpdate, mockOnEdgeUpdate),
    ...
    const newGraph = {
      nodes: [
        { id: 'new_node', type: 'load', position: { x: 50, y: 50 }, data: {} },
      ],
      edges: [],
    }
  ...
  test('should handle callback changes correctly', async () => {
    const { result, rerender } = renderHook(
      ({ onNodeUpdate }) =>
        useFaultParticleAnimation(mockGraph, onNodeUpdate, mockOnEdgeUpdate),
