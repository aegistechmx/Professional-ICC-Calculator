...
export function useSmartSnap(nodes, options = {}) {
    ...
    (draggedNode, currentPosition) => {
      ...
      if (mode === SNAP_MODES.SMART) {
        ...
        if (spacing && !snapped.x) {
          ...
          const consistentX = findConsistentPositionX(
            nodes,
            draggedNode,
            x,
            spacing
        ...
        if (spacing && !snapped.y) {
          ...
          const consistentY = findConsistentPositionY(
            nodes,
            draggedNode,
            y,
            spacing
  ...
  function detectConsistentSpacing(nodes, excludeNode, tolerance) {
    ...
    for (let i = 0; i < otherNodes.length; i++) {
      for (let j = i + 1; j < otherNodes.length; j++) {
        const spacing = Math.abs(
          otherNodes[i].position.x - otherNodes[j].position.x
        )
    ...
    for (let i = 0; i < otherNodes.length; i++) {
      for (let j = i + 1; j < otherNodes.length; j++) {
        const spacing = Math.abs(
          otherNodes[i].position.y - otherNodes[j].position.y
        )
        if (spacing > tolerance) {
          spacings.push(spacing)
        }
    ...
    const mostCommon = Object.entries(grouped).sort((a, b) => b[1] - a[1])[0]
