...
export default function useFaultParticleAnimation(
  graph,
  onNodeUpdate,
  onEdgeUpdate
) {
    ...
    const engine = new FaultParticleEngine(canvasRef.current, {
      ...
      onFaultStart: (faultId, nodeId, Icc) => {
        setActiveFaults(prev =>
          new Map(prev).set(faultId, { nodeId, Icc, startTime: Date.now() })
        )
        setIsAnimating(true)
      },
   ...
   */
  const handleBreakerTrip = useCallback(
    (breakerId, breakerData, graph) => {
      ...
      breakerEffectsRef.current?.handleBreakerTrip(
        breakerId,
        breakerData,
        graph
      )
   ...
   */
  const startFaultParticleAnimation = useCallback(
    (nodeId, Icc) => {
