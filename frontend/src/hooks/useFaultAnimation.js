...
export default function useFaultAnimation(graph, onNodeUpdate, onEdgeUpdate) {
   ...
   */
  const getUpstreamPath = useCallback(
    nodeId => {
   ...
   */
  const extractBreakersFromPath = useCallback(
    path => {
   ...
   */
  const startFaultAnimation = useCallback(
    (nodeId, Icc) => {
    ...
    if (activeFault) {
      ...
      activeFault.breakers.forEach(breaker => {
        onNodeUpdate?.(breaker.id, {
          style: {},
          data: {
            ...graph.nodes.find(n => n.id === breaker.id)?.data,
            status: null,
            tripTime: null,
          },
        })
      ...
      onNodeUpdate?.(activeFault.nodeId, {
        style: {},
        data: {
          ...graph.nodes.find(n => n.id === activeFault.nodeId)?.data,
          status: null,
          Icc: null,
        },
      })
