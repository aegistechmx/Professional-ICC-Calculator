...
export const TCCLiveChart = ({
  ...
  const chartCurves = useMemo(() => {
    ...
    return breakerNodes.map(breaker => {
      ...
      return {
        ...
        color: getTCCColor(
          getTCCStatus(nodeResult?.I || 0, pickup, instantaneous)
        ),
  ...
  const getChartCoordinates = useCallback(
    (current, time) => {
      ...
      const currentLog =
        (Math.log10(current) - Math.log10(minCurrent)) /
      ...
      const maxTime = 10000
      const timeLog =
        (Math.log10(time) - Math.log10(minTime)) /
  ...
  const drawAxes = useCallback(
    ctx => {
      ...
      ctx.lineTo(
        width - chartConfig.padding.right,
        height - chartConfig.padding.bottom
      )
      ctx.stroke()
      ...
      currentValues.forEach(current => {
        ...
        ctx.fillText(
          ...
          x - 10,
          height - chartConfig.padding.bottom + 20
        )
      })
  ...
  const drawCoordination = useCallback(
    ctx => {
