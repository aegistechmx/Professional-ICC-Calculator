  ...
  return (
    <div className="timeline-container" style={style}>
      ...
      <div className="relative">
        ...
        <div className="relative mt-2">
          {eventMarkers.map((marker, index) => (
            <EventMarker key={index} marker={marker} />
          ))}
        </div>
      ...
      <div className="mt-4 space-y-2">
        {eventMarkers.map((marker, index) => (
          <EventItem key={index} marker={marker} isActive={marker.isCurrent} />
        ))}
      </div>
    </div>
  )
      ...
      <div className="flex-1">
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm text-gray-500">
            {marker.normalizedTime.toFixed(3)}s
          </span>
        </div>
...
function getEventColor(type) {
  ...
  if (
    typeLower.includes('fault') ||
    typeLower.includes('falla') ||
    typeLower === 'fault_detected'
  ) {
    return '#ef4444' // Red
  }
  ...
  if (
    typeLower.includes('trip') ||
    typeLower.includes('breaker') ||
    typeLower.includes('relay_trip')
  ) {
    return '#f97316' // Orange
  }

  if (
    typeLower.includes('stable') ||
    typeLower.includes('estable') ||
    typeLower.includes('convergence')
  ) {
    return '#22c55e' // Green
  }
    ...
    <div className="sequence-diagram p-4 bg-white rounded-lg shadow">
      ...
      <div className="space-y-4">
        {processedEvents.map((event, index) => {
          ...
          const isNext =
            index > 0 &&
            currentTime < event.normalizedTime &&
            ...
            <div key={index} className="flex items-center gap-4">
              ...
              <div className="flex-1 relative">
                ...
                <div
                  ...
                  style={{
                    backgroundColor: isPast || isNext ? color : '#d1d5db',
                  }}
                />
              </div>

              <div
                className={`text-sm ${isPast ? 'text-gray-800' : isNext ? 'text-blue-600 font-semibold' : 'text-gray-400'}`}
              >
                {event.type}
              </div>

              <div
                className={`text-xs ${isPast ? 'text-gray-600' : 'text-gray-400'} flex-1`}
              >
