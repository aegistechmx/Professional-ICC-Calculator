    ...
    deltaTime => {
      ...
      if (now - lastSpawn > 50) {
        ...
        simulation.flows.forEach(flow => {
          if (flow.current > 10) {
            // Solo generar partículas para flujos significativos
...
export const useSimulationEngine = () => {
  ...
  if (!context) {
    throw new Error(
      'useSimulationEngine debe usarse dentro de SimulationEngine'
    )
  }
  return context
}
...
export const useFlowAnimation = edgeId => {
  ...
  const flow = useMemo(() => flows.find(f => f.id === edgeId), [flows, edgeId])
