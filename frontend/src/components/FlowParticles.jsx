...
export const FlowParticles = ({ edgeId, width = 100, height = 20 }) => {
  ...
  const { isVisible } = useFlowAnimation(edgeId)
  ...
  const animate = useCallback(() => {
    ...
    particles.forEach(particle => {
      ...
      const x =
        edgePath.start.x + (edgePath.end.x - edgePath.start.x) * progress
      const y =
        edgePath.start.y + (edgePath.end.y - edgePath.start.y) * progress
...
export const ReactFlowParticles = ({ edgeId }) => {
  ...
  }, [particles, isVisible, edgeId, nodes, edges])
      ...
      <div className="bg-gray-50 rounded p-3">
        ...
        <div className="space-y-1 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span
              className={`font-medium ${showParticles ? 'text-green-600' : 'text-gray-500'}`}
            >
...
export const ParticleCanvasOverlay = () => {
  ...
  const { showParticles, nodes, edges, simulation } = useGraphStore()
...
FlowParticles.propTypes = {
  edgeId: PropTypes.string.isRequired,
  width: PropTypes.number,
  height: PropTypes.number,
}

ReactFlowParticles.propTypes = {
  edgeId: PropTypes.string.isRequired,
}

FlowParticleMarker.propTypes = {
  edgeId: PropTypes.string.isRequired,
}

ParticleControlPanel.propTypes = {
  // No props required - uses store directly
}

ParticleCanvasOverlay.propTypes = {
  // No props required - uses store directly
}
