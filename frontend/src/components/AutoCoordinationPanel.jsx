...
export const AutoCoordinationPanel = () => {
  ...
  const { nodes, edges, results, generateTCCCurves, autoCoordinateBreakers } =
    useGraphStore()
  ...
  const analyzeCoordination = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < breakerNodes.length - 1; i++) {
        ...
        const margin = calculateCoordinationMargin(
          upCurve.curve,
          downCurve.curve
        )
      ...
      const goodPairs = results.breakerAnalysis.filter(
        a => a.status === 'good'
      ).length
  ...
  useEffect(() => {
    if (autoMode && coordinationResults) {
      const hasIssues = coordinationResults.issues.some(
    ...
    <div className="bg-white rounded-lg shadow-md p-4 border border-gray-200">
          ...
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-lg font-bold ${
                coordinationResults.overallScore > 80
                  ? 'text-green-600'
                  : coordinationResults.overallScore > 60
                    ? 'text-yellow-600'
                    : 'text-red-600'
              }`}
            >
                ...
                {coordinationResults.coordinationMatrix.map((pair, index) => (
                  <tr key={index} className="border-b">
                    <td className="px-2 py-1">
                      {breakerNodes.find(b => b.id === pair.upstream)?.data
                        ?.label || pair.upstream}
                    </td>
                    <td className="px-2 py-1">
                      {breakerNodes.find(b => b.id === pair.downstream)?.data
                        ?.label || pair.downstream}
                    </td>
                    ...
                    <td className="px-2 py-1 text-center">
                      <span
                        className={`px-2 py-1 rounded text-xs ${getStatusColor(pair.status)}`}
                      >
                        {pair.status}
                      </span>
                    </td>
      ...
      {selectedBreaker && (
        <div className="mb-4 p-3 bg-gray-50 rounded">
          <h4 className="text-sm font-medium text-gray-700 mb-2">
            Detalles:{' '}
            {breakerNodes.find(b => b.id === selectedBreaker.upstream)?.data
              ?.label || selectedBreaker.upstream}
          </h4>
          <div className="space-y-1 text-xs">
            {selectedBreaker.testPoints.map((point, index) => (
              <div key={index} className="flex justify-between">
                ...
                <span
                  className={
                    point.acceptable ? 'text-green-600' : 'text-red-600'
                  }
                >
