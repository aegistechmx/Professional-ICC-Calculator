...
export const CoordinationPanel = () => {
  ...
  const { nodes, edges, results, simulation, triggerFault, clearFault } =
    useGraphStore()
    ...
    if (simulation.fault && protectionSystem) {
      ...
      const graph = {
        ...
        edges: edges.map(edge => {
          ...
          return {
            ...edge,
            breaker: protectionSystem.getBreakerState(
              edge.data?.breaker ||
                nodes.find(
                  n =>
                    n.type === 'breaker' &&
  ...
  // === CONTROL DE FALLA ===
  const handleFaultTrigger = useCallback(
    nodeId => {
      triggerFault(nodeId)
    },
    [triggerFault]
      ...
      <div className="mb-4 p-3 bg-gray-50 rounded">
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className={`text-sm font-bold ${
              coordinationResults ? 'text-red-600' : 'text-green-600'
            }`}
          >
        ...
        {coordinationResults && (
          <div className="text-xs text-gray-600">
            Breakers: {coordinationResults.summary.totalBreakers} | Disparados:{' '}
            {coordinationResults.summary.tripped} | Bloqueados:{' '}
            {coordinationResults.summary.blocked}
          </div>
        )}
      </div>
            ...
            return (
              <div className="p-3 bg-gray-50 rounded">
                ...
                <div className="text-xs text-gray-600">
                  ...
                  <div>
                    Bloqueados: {(quality.blockedRatio * 100).toFixed(1)}%
                  </div>
                </div>
              </div>
            )
            ...
            {coordinationResults.issues.map((issue, index) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  >
                    {issue.type === 'coordination_violation'
                ...
                <div className="text-xs text-gray-600 mt-1">
                  ...
                  <div>
                    Margen: {issue.margin.toFixed(3)}s (min: {issue.minMargin}s)
                  </div>
                </div>
              </div>
            ))}
            ...
            <div>
              ...
              <span
                className={`ml-2 font-medium ${
                  selectedBreaker.breaker.tripped
                    ? 'text-red-600'
                    : selectedBreaker.breaker.blocked
                      ? 'text-orange-600'
                      : selectedBreaker.breaker.instBlocked
                        ? 'text-yellow-600'
                        : 'text-green-600'
                }`}
              >
