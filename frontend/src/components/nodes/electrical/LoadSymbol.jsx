...
export default function LoadSymbol({ data, selected }) {
  const { status, label, I_carga, longitud, tipo = 'motor' } = data || {}
  ...
  return (
    <div
      className={`electrical-node load ${selected ? 'selected' : ''} ${isOverload ? 'overload' : ''}`}
    >
      ...
      >
        <svg width="60" height="60" viewBox="0 0 60 60" className="iec-symbol">
          ...
          {isMotor ? (
            ...
            <>
              <line
                ...
                strokeWidth="2"
              />
              <polygon points="25,35 30,42 35,35" fill={getStatusColor()} />
            </>
          )}
          ...
          {isOverload && <circle cx="48" cy="12" r="4" fill="#dc2626" />}
        </svg>
      ...
      {/* Indicador de sobrecarga */}
      {isOverload && <div className="overload-indicator">SOBRECARGA</div>}
    </div>
  )
}
