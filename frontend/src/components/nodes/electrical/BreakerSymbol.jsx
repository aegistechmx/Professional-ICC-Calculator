...
export default function BreakerSymbol({ data, selected }) {
  const { status, label, In, Icu } = data || {}
  ...
  return (
    <div
      className={`electrical-node breaker ${selected ? 'selected' : ''} ${isTripped ? 'tripped' : ''}`}
    >
      ...
      >
        <svg width="60" height="60" viewBox="0 0 60 60" className="iec-symbol">
          ...
          {isTripped && <circle cx="30" cy="30" r="3" fill="#dc2626" />}
        </svg>
      ...
      <div className="node-info">
        ...
        <div className="node-values">
          ...
          {Icu && <span className="icu">Icu: {formatValue(Icu, 'kA', 1)}</span>}
        </div>
      </div>

      {/* Indicador de disparo */}
      {isTripped && <div className="trip-indicator">DISPARADO</div>}
    </div>
  )
}
