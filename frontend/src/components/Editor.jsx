...
function Editor() {
  ...
  }, [getNodes, getEdges, setReactFlowNodes, setReactFlowEdges, setSelectedNode, setSelectedEdge]);
  ...
  const edgesEqual = (a, b) => {
    ...
    if (!a || !b) return false;
    if (a.length !== b.length) return false;
    return a.every((edge, i) => edge.id === b[i]?.id &&
  ...
  useEffect(() => {
    if (Array.isArray(storeNodes) && !nodesEqual(storeNodes, lastSyncedNodes.current)) {
  ...
  useEffect(() => {
    if (Array.isArray(storeEdges) && !edgesEqual(storeEdges, lastSyncedEdges.current)) {
