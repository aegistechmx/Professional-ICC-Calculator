...
export const FaultAnimationLayer = ({ width = 1200, height = 800 }) => {
  ...
  const { nodes, edges, results, simulation } = useGraphStore()
  ...
  const initFaultEffects = useCallback(
    faultNodeId => {
      const effects = []
  ...
  const drawFaultEffects = useCallback(
    ctx => {
      ...
      faultEffectsRef.current.forEach(effect => {
        ...
        const pulse =
          Math.sin(elapsed * faultConfig.pulseFrequency * 0.001) * 0.3 + 0.7
...
export const FaultControlPanel = () => {
  const { nodes, edges, simulation, triggerFault, clearFault, tripBreaker } =
    useGraphStore()

  const isUpstreamBreaker = useCallback(
    (breakerId, nodeId) => {
