...
export const NodeStatusIndicator = ({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {/* Optional label */}
      {showLabel && (
        <span className="text-xs font-medium text-gray-600">{nodeLabel}</span>
      )}
    </div>
  )
}
        ...
        <div className="bg-red-50 border border-red-200 rounded p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-red-800">
              Falla activa en nodo:{' '}
              {nodes.find(n => n.id === simulation.fault)?.data?.label ||
                simulation.fault}
            </span>
          </div>
        </div>
...
export const NodeStatusBadge = ({ nodeId, className = '' }) => {
  const { status, color, isFault } = useNodeStatus(nodeId)
  const { nodes } = useGraphStore()
  ...
  return (
    ...
    >
      ...
      {node?.data?.label && (
        <span className="text-xs opacity-75">({node.data.label})</span>
      )}
    </div>
  )
}
...
export const NodeDetailsPanel = ({ nodeId }) => {
  ...
  const statusConfig = {
    ...
    yellow: {
      label: 'Precaución',
      description: 'Carga elevada o baja tensión',
    },
    orange: {
      label: 'Breaker Trip',
      description: 'Breaker ha operado por protección',
    },
          ...
          {node.data?.I_nominal && nodeResult?.I && (
            <div className="mt-3">
              <div className="flex justify-between text-xs text-gray-500 mb-1">
                <span>Carga</span>
                <span>
                  {Math.round((nodeResult.I / node.data.I_nominal) * 100)}%
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
