...
describe('Particle System Performance Tests', () => {
  ...
  describe('Memory Usage Performance', () => {
    test('should not exceed memory budget with high particle counts', () => {
      ...
      expect(afterUpdateStats.memoryUsage).toBeLessThanOrEqual(
        afterSpawnStats.memoryUsage * 1.5
      ) // Trail memory shouldn't double
    })
  ...
  describe('Stress Tests', () => {
    ...
    test('should handle concurrent fault animations', () => {
      ...
      for (let i = 0; i < 5; i++) {
        const faultId = engine.emitFaultParticles(
          graph,
          `n${i + 1}`,
          5000 + i * 1000
        )
