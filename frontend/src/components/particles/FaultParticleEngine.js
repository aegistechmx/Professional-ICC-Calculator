...
export class FaultParticleEngine {
  ...
  evaluateBreakerTrips(upstreamPath, graph, Icc) {
    upstreamPath.forEach((edge, index) => {
      ...
      if (
        breakerNode &&
        breakerNode.type === 'breaker' &&
        breakerNode.data?.ratings
      ) {
  ...
  handleBreakerTrip(breakerId, breakerData, graph, faultCurrent) {
    ...
    if (this.breakerEffects) {
      this.breakerEffects.handleBreakerTrip(
        breakerId,
        breakerData,
        graph,
        faultCurrent
  ...
  updateOptions(newOptions) {
    if (newOptions.particleSystem) {
      ...
      Object.assign(
        this.particleSystem.globalOptions,
        newOptions.particleSystem
      )
    }
