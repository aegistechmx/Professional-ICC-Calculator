...
describe('Store API Integration', () => {
  ...
  describe('API Calls', () => {
    it('should make correct API call structure', async () => {
      ...
      const result = await mockedAxios.post('http://localhost:3002/powerflow/run', {
        nodes: [],
        edges: [],
        options: {
          Sbase_MVA: 100,
  ...
  describe('State Management Logic', () => {
    it('should handle node updates', () => {
      let nodes = [];
      const setNodes = (newNodes) => { nodes = newNodes; };
    ...
    it('should handle edge updates', () => {
      let edges = [];
      const setEdges = (newEdges) => { edges = newEdges; };
    ...
    it('should handle selected node updates', () => {
      ...
      const setSelectedNode = (node) => { selectedNode = node; };
