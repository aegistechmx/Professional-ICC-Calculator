...
} from '../utils/cascadeSimulation'
import { playTimeline, getTimelineStats } from '../utils/timelinePlayback'
import {
  extractBranches,
  calculateBranchImpedance,
} from '../utils/extractBranches'
...
export const useStore = create((set, get) => ({
  ...
  clearValidationErrors: () =>
    set({ validationErrors: [], lastValidationTime: null }),
  ...
  addInvalidConnection: (sourceId, targetId, reason) =>
    set(state => ({
      invalidConnections: [
    ...
    const interval = setInterval(() => {
      ...
      set(state => ({
        currentTime: Math.min(
          state.currentTime + 0.01 * state.playbackSpeed,
          state.maxTime
        ),
      }))
    }, 10) // 10ms update rate
  ...
  calculateBranches: async () => {
    try {
      ...
      const branchResults = branches.map(branch => {
        ...
        const sourceVoltage =
          branch.source.data?.parameters?.secundario ||
          branch.source.data?.parameters?.voltaje ||
          480
  ...
  generatePDF: async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || error.message || 'Error al generar PDF'
      throw new Error(`❌ ${errorMessage}`)
    }
  },
  ...
  calculatePowerFlow: async (options = {}) => {
    ...
    try {
      ...
      if (result.success && result.buses) {
        const visualResults = applySimulationResults(
          { nodes, edges },
          {
  ...
  calculateShortCircuitFromGraph: async () => {
    ...
    try {
      ...
      if (error.response?.status === 400) {
        errorMessage =
          '❌ Datos inválidos para el cálculo de cortocircuito. Verifica los parámetros.'
  ...
  calculateFaultWithMotors: async () => {
    try {
      ...
      const errorMessage =
        error.message || 'Error al calcular contribución de motores'
      throw new Error(`❌ ${errorMessage}`)
    }
  },
}))
