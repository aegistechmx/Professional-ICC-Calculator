    ...
    (set, get) => ({
      ...
      calculateSystemRealtime: async () => {
        ...
        try {
          ...
          console.warn(
            '[STORE] Error en cálculo en tiempo real:',
            error.message
          )
        }
      },
      ...
      getRealtimeResult: async graphHash => {
        try {
          ...
          console.warn(
            '[STORE] Error obteniendo resultado en tiempo real:',
            error.message
          )
          return null
        }
          ...
          reader.onload = event => {
            try {
              ...
              if (!projectData.nodes || !projectData.edges) {
                throw new Error(
                  'Archivo inválido: no contiene nodos o conexiones'
                )
              }
      ...
      updateSimulation: simulationData =>
        set(state => ({
          simulation: {
            ...state.simulation,
            ...simulationData,
      ...
      calculateFlows: () => {
        ...
        const flows = edges.map(edge => {
          const edgeResult = results.flujos.find(
            f => f.from === edge.source && f.to === edge.target
          )
      ...
      triggerFault: nodeId => {
        ...
        const propagation = simulateFaultPropagation(
          nodeId,
          nodes,
          edges,
          results
            ...
            simulation: {
              ...
              breakers: {
                ...
                [breakerId]: {
                  ...
                  position: state.nodes.find(n => n.id === breakerId)
                    ?.position || { x: 0, y: 0 },
                },
              },
              lastUpdate: new Date().toISOString(),
      ...
      resetBreaker: breakerId => {
        set(state => ({
          simulation: {
            ...state.simulation,
            breakerTrips: state.simulation.breakerTrips.filter(
              id => id !== breakerId
            ),
            breakers: {
              ...state.simulation.breakers,
      ...
      isEdgeBlocked: edgeId => {
        ...
        const breakerNodes = nodes.filter(n => n.type === 'breaker')
        const edgeBreaker = breakerNodes.find(
          breaker =>
            breaker.id === edge.source ||
            breaker.id === edge.target ||
      ...
      generateTCCCurves: () => {
        ...
        breakerNodes.forEach(breaker => {
          ...
          tccCurves[breaker.id] = {
            ...
            curve: generateTCCTimeCurrentCurve(
              pickup,
              inst,
              breaker.data?.curve || 'IEC'
            ),
        ...
        while (hasChanges && iterations < maxIterations) {
          ...
          for (let i = 0; i < breakerNodes.length - 1; i++) {
            ...
            if (
              downCurve &&
              upCurve &&
              curvesOverlap(downCurve.curve, upCurve.curve)
            ) {
